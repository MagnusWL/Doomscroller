...
//
// The browser draws a hard line here, and it shapes this whole file:
//   - Banner slides are Adcash iframes on another origin. Nothing in JS can
//     rasterize those — not canvas, not any library. Their pixels are gone.
//   - A video's frames can be drawn to a canvas, but the canvas taints unless
//     the media server sends CORS headers, and toDataURL then throws.
// Showing a cross-origin video is always allowed though — only reading it back
// is blocked. So a failed capture costs a still, not the item: the inventory
...
// Returns null when there's nothing worth keeping: a banner, or a video slide
// whose ad hasn't filled yet.
