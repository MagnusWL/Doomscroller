...
// Rotating the sources isn't enough on its own. Adcash's two zones draw on one
// pool of about three videos, so the neighbouring slides that ask each of them
// still land on the same creative often. This is what actually stops you seeing
// the same ad twice in a row — measured, the two zones handed back an identical
// creative on 2 of every 8 consecutive slides.
const RECENT_MAX = 3;
const recent = [];

// The whole path, because neither network alone is enough to tell creatives
// apart: Adcash names every file low_640x360_1000.mp4 and puts the identity in
// the directory, HilltopAds shares a directory and puts it in the filename.
const identity = url => {
  try {
...
function remember(ad) {
  recent.push(identity(ad.media.url));
...
export async function loadVideoAd() {
  ...
  for (let i = 0; i < VAST_TAGS.length; i++) {
    ...
    if (!ad) continue;
    if (!recent.includes(identity(ad.media.url))) {
      remember(ad);
      return ad;
    }
    // Hold it and ask elsewhere. Only three creatives exist at Adcash, so every
    // source having nothing new is a real outcome, and a repeat beats a blank.
    repeat = repeat || ad;
  }

  if (repeat) remember(repeat);
  return repeat;
}
