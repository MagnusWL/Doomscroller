...
// Originalen er lib/coin-sack-engine.js. Det her er den samme fil med to
// ændringer, begge fordi den skal kunne loades af en almindelig <script>-tag:
// `export class CoinSack` blev en klasse hængt på window, og alting er pakket
// ind i en funktion.
//
// Indpakningen er ikke pynt. Denne fil og frames.js erklærer begge en `const
// GOLD`. Som moduler har de hver sit rum; som almindelige scripts deler de ét,
// og så dør den fil der læses sidst med "Identifier 'GOLD' has already been
// declared" — hele filen, uden en lyd i konsollen.
//
// Hvorfor så ikke bare moduler: de nægter at loade over file://, og hele
// pointen med lab'et er at det åbner ved dobbeltklik uden server.
//
// Find fejl i lib/coin-sack-engine.js og kør kopien igen — se README.md.
(function () {
...
class CoinSack {
  ...
  _pixelPalette() {
    if (this.style === 'pixel2') return {
      ...
      gold: ['#fff6d6', '#ffe08a', '#f5be3c', '#c6871f', '#7e5212'],
    };
    return {
      p: 7,
      tones: ['#b98a44', '#8a6228', '#553a18'],
      gold: ['#fff2b0', '#ffd24a', '#e0a02a', '#9c6a18', '#5e3d0e'],
    };
  }
