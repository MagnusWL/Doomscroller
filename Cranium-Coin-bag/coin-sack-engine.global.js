// ── GENERERET fra coin-sack-engine.js — ret ikke noget her ──────────────────
// Samme motor, men som et almindeligt script: `export class` er væk, og
// klassen hænger på window.CoinSack i stedet.
//
// Hvorfor to udgaver: ES-moduler nægter at loade over file://, så example.html
// kunne ikke åbnes ved dobbeltklik hvis den skulle importere. Bruger dit spil
// moduler (import/export), så brug coin-sack-engine.js. Bruger det almindelige
// <script>-tags, så brug den her.
//
...
class CoinSack {
  constructor(canvas, opts = {}) {
    ...
    // the soft ellipse under the container. On by default; the skull package
    // turns it off.
    this.groundShadow = opts.groundShadow !== false;
  ...
  _drawContainerBack(ctx, fill, now) {
    const g = this._geom();
    // ground shadow — optional since the skull package: a soft ellipse fights
    // a hard-edged pixel look, and the game may draw its own grounding
